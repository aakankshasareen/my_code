...
//    }
    var query = connection.query("SELECT SQL_CALC_FOUND_ROWS su.*, CONCAT('supportdoc/',created_by,'/',doc_name)as document_path, @count:=@count+1 AS serial_number FROM support su , (SELECT @count:="+offset+") AS X WHERE su.status != 2 " + searchQuery + "  LIMIT " + offset + ", " + limit, function (error, data) {
        if (error) {
                        ...
                        if (error) {
                            ...
                            let query = connection.query(sql2, function(error, userdata){
                                ...
                                //email_helper.mail_template("we012", "suneel.gupta@sofocle.com", data, req, res)
                                    ...
                                    email_helper.mail_template("MT125", userEmail , data, req, res)
...
exports.changeStatus= function(req, res){
    var id = req.params.id;
    let sql = "SELECT description, status, id from support WHERE id=? ";
    let value = [id];
    sql = mysql.format(sql, value);
    connection.query(sql, function(error, result){
        if(error){
           res.json({"success": false, "message": "error", error});  
            ...
            res.json({"success": true, "message": "Support Details  Found", result});
       }
