...
exports.ltcUserAccount = function (req, res) {
    ...
    connection.query(sql, function (error, data) {
        if (error) {
            ...
            if (data[0].kyc_status != 2) {
                ...
                //console.log(data)
                let sql1 = "SELECT * FROM user_crypto_address WHERE user_id ='" + data[0].id + "'AND crypto_type='LTC'";
                // console.log(sql1)
...
/** Older function with pending response for deposits more than one case. */
exports.testltcBalance = function (req, res) {
    // var tranarray=[];
    console.log('Call in LTC balance');
                        ...
                        if (err) {
                            ...
                            connection.query(tranLog_sql, function (error, masterfinal) {
                                ...
                                if (error) {
                                    ...
                                    let masterWallet_sql = mysql.format("UPDATE master_customer_wallet SET total_amount = `total_amount` + ? WHERE customer_id=? AND currency_code='LTC'", [amount, custId]);
                                    ...
                                    connection.query(masterWallet_sql, function (error, masterfinal) {
                                        ...
                                        if (err) {
                                            ...
                                            let customerWallet_sql = mysql.format("UPDATE customer_wallet SET total_amount =`total_amount` + ?  WHERE customer_id= ? AND currency_code='LTC'", [amount, custId]);
                                                ...
                                                if (err) {
                                                    ...
                                                    connection.query(deposit_sql, function (error, ress) {
                                                        ...
                                                        if (err) {
                                                            ...
                                                            var logData = {
                                                                "user_id": userId,
                                                                "activity_description": 'money LTC added to wallet',
                                                                "activity_type": 005,
                    ...
                    function (err) {
                        if (err) {
                            ...
                            connection.commit(function (err) {
                                ...
                                resolve('LTC deposit to the wallet');
                                console.log("LTC deposit to the wallet")
                            });
    ...
    connection.query(sql, function (error, data) {
        if (error) {
            ...
            let sql1 = mysql.format("SELECT * FROM user_crypto_address WHERE user_id = ? AND crypto_type='LTC'", [data[0].created_by]);
            console.log(sql1)
            connection.query(sql1, function (error, dta) {
                if (error) {
                    res.json({ success: false, message: "error", error })
                    ...
                    res.json({ success: false, message: "LTC address is not created" })
                } else {

                    console.log("label", req.decoded.email)
                    ltc_block_io.get_transactions({ 'type': 'received', 'addresses': dta[0].crypto_address }, function (error, addr) {
                        ...
                        if (error) {
                            ...
                            // let sqlSelectTimeStamp = mysql.format("Select id from customer_deposite where customer_id=? AND transaction_hash=? AND transaction_timestamp=? AND currency_code='LTC' order by id desc limit 1", [data[0].id, transaction_hash, timestamp1]);
                            ...
                            //     } else {
                            let countTran_sql = mysql.format("Select COUNT( * ) as count_tran from transaction_log where customer_id = ? AND currency_code='LTC'", [data[0].id])
                            connection.query(countTran_sql, function (error, countfinal) {
                                if (error) {
                                    ...
                                    let sql2 = mysql.format("SELECT * FROM master_customer_wallet WHERE customer_id = ? AND currency_code='LTC'", [data[0].id]);
                                    console.log(sql2)
...
exports.ltcDepositHistory = function (req, res) {
    var url;
    var email = req.decoded.email;
        ...
        if (error) {
            ...
            connection.query(addr_sql, function (error, adddata) {
                if (error) {
                    ...
                    console.log(url)
                    //url = 'https://chain.so/api/v2/get_tx_received/LTCTEST/' + adddata[0].crypto_address + ''
                    //url = 'https://chain.so/api/v2/get_tx_received/LTCTEST/2NAWaVXW2agXVC2GxjWRi3h7FAiBhb6BmRq'
...
exports.ltcBalance = function (req, res) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            connection.beginTransaction(function (err) {
                ...
                async.forEachOfSeries(transdata, function (error, i, inner_callback) {
                    ...
                    let select_tranLog_sql = mysql.format("Select * from transaction_log where transactionhash =? AND currency_code='LTC' AND customer_id= ? ", [transaction_hash, custId]);
                    connection.query(select_tranLog_sql, function (err, checktran) {
                        if (err) {
                            //  console.log("presentcustomerwalletdata wallet error1", err)
                            inner_callback(err);
        ...
        if (error) {
            ...
            connection.query(sql1, function (error, dta) {
                if (error) {
                    ...
                    let getAlreadyTransactionHash = mysql.format("select transactionhash from transaction_log where customer_id=? AND currency_code='LTC'", [data[0].id]);
