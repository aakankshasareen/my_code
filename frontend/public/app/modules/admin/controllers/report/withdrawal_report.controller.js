admin.controller("widthdrawalReportCtrl", ['$rootScope', '$scope', '$timeout', 'loginService', 'dashboardService', 'AuthService', 'adminService', '$state', 'toastr', 'paginationFactory', '$stateParams', '$window', 'uiGridConstants',
    function($rootScope, $scope, $timeout, loginService, dashboardService, AuthService, adminService, $state, toastr, paginationFactory, $stateParams, $window, uiGridConstants) {
        ...
        var vm = this;
        var searchParams = [];
        $scope.pageHeading = "Withdrawal Report";
        $scope.orderCoulmn = '';
        ...
        $scope.getGridData = function() {
            $scope.gridOptions = {
                ...
                exporterCsvFilename: 'DepositReport.csv',
                ...
                columnDefs: [
                    { field: 'serial_number', name: 'S.No.', width: '5%', enableSorting: false, enableFiltering: false,headerCellClass:'text-center',cellClass:'text-center' },
                    {
                        field: 'email',
                        filter: {
                            placeholder: 'Search...'
        ...
        $scope.export = function(format) {
            ...
            var result = adminService.getWithdrawalReport(data);
            result.then(
                function(response) {
        ...
        $scope.drawGrid = function() {
            ...
            var data = angular.merge({}, object1);
            var result = adminService.getWithdrawalReport(data);
            result.then(
                function(response) {
